...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = x.view(batch_size, self.n_features * self.embed_size) # (batch_size, n_features * embed_size)
    ...
    def forward(self, w):
        ...
        logits = self.embedding_lookup(w) # (batch_size, n_features * embed_size)
        logits = torch.matmul(logits, self.embed_to_hidden_weight) + self.embed_to_hidden_bias # (batch_size, hidden_size)
        logits = F.relu(logits) # (batch_size, hidden_size)
        logits = self.dropout(logits) # (batch_size, hidden_size)
